...
- [ ] PS / HE-AACv2 synthesis remains unimplemented. Progress (2026-09-24): explicit HE-AAC AOT 5 hierarchical ASC signaling is implemented and tested (core/extension rates, `sbrPresentFlag`, reserved explicit-frequency rejection); AOT 29 metadata is parsed and exposed through `AudioSpecificConfig`, but `AacDecoder` rejects it rather than silently downmixing. The in-band PS IID/ICC/IPD/OPD payloads are now parsed and validated with allocation-free state, canonical VLC decoding, nested-extension framing, and malformed-input fallback; QMF-domain PS stereo synthesis remains open, so output currently falls back to mono.
