...
- [ ] **Opus encoder** (user-confirmed first target — hybrid SILK/CELT encoding, bitrate control, psychoacoustic tuning; reuses existing range coder/CELT/SILK decode infrastructure). **In progress** — CELT encoder supports mono *or stereo*, fullband, CBR, all 4 frame sizes; a top-level `OggOpusEncoder` (implementing the shared `Encoder` trait, writing real `.opus`/Ogg files) landed 2026-09-23. **Blocking correctness gap found 2026-09-23, not yet fixed**: the CELT CBR encoder can silently overshoot its requested byte budget at nearly every bitrate except two specific validated values (160 bytes/20ms mono, 320 stereo), corrupting decode with no error signal — see "Session log (2026-09-23, continued): CELT CBR encoder can silently overshoot its byte budget" below for the full root-cause analysis and the `#[ignore]`d regression test pinning it. This is now the top-priority remaining item — a real, general-purpose Opus encoder isn't usable until it's fixed. See "Opus CELT encoder — foundation (2026-09-21)" below for the mono foundation, "Session log (2026-09-22, continued): stereo support" for the stereo work, and the two 2026-09-23 session logs for the exact-digital-silence fix and the `OggOpusEncoder`/CBR-overshoot work.
...
**Why this wasn't fixed this session**: a real fix needs one of:
- A genuine hard output-size cap enforced *during* entropy coding (mirroring libopus's `ec_enc_shrink`, which constrains the range encoder's storage budget structurally, not by estimating after the fact and hoping) — this crate's `RangeEncoder` has no such mechanism at all (confirmed: no `storage`/max-size field exists; output is an unbounded `Vec<u8>`), so adding one is a real, non-trivial change to the range coder's carry-propagation/`done()` logic, not a local patch.
- Or a retry-with-smaller-budget loop in `encode_frame_impl` (encode speculatively, check the real size, re-encode with a tighter budget if it overshot) — architecturally simpler to reason about but requires either buffering enough state to re-run the whole per-band quantization pipeline, or restructuring it to be retriable.

Both are bigger than remaining session budget allowed for a change this consequential (it affects the *entire* CBR encode path, mono and stereo alike, at nearly every bitrate).
