...
static void ps_stage_dump(int tag, const void *buf, size_t nb)
{
    const char *path = getenv("PS_STAGE_DUMP");
    FILE *f;
    if (!path) return;
    f = fopen(path, "ab");
    if (!f) return;
...
int AAC_RENAME(ff_ps_apply)(PSContext *ps, INTFLOAT L[2][38][64], INTFLOAT R[2][38][64], int top)
{
    ...
    ps_stage_dump(1, Lbuf, sizeof(ps->Lbuf));
    decorrelation(ps, Rbuf, (const INTFLOAT (*)[32][2]) Lbuf, is34);
    ps_stage_dump(2, Rbuf, sizeof(ps->Rbuf));
    stereo_processing(ps, Lbuf, Rbuf, is34);
    ps_stage_dump(3, Lbuf, sizeof(ps->Lbuf));
    ps_stage_dump(4, Rbuf, sizeof(ps->Rbuf));
...
/* ---- harness-only addition (not upstream): dump generated tables ---- */
void dump_tables(const char *outdir);
void dump_tables(const char *outdir)
{
    static const struct { const char *name; const void *buf; int count; } tabs[] = {
        { "q_fract_allpass", Q_fract_allpass, 2 * 50 * 3 * 2 },
        { "phi_fract",       phi_fract,       2 * 50 * 2 },
        { "ha",              HA,              46 * 8 * 4 },
        { "hb",              HB,              46 * 8 * 4 },
        { "pd_re_smooth",    pd_re_smooth,    512 },
        { "pd_im_smooth",    pd_im_smooth,    512 },
        { "f20_0_8",         f20_0_8,         8 * 8 * 2 },
        { "f34_0_12",        f34_0_12,        12 * 8 * 2 },
        { "f34_1_8",         f34_1_8,         8 * 8 * 2 },
        { "f34_2_4",         f34_2_4,         4 * 8 * 2 },
    };
    for (unsigned i = 0; i < sizeof(tabs) / sizeof(tabs[0]); i++) {
        char path[512];
        FILE *f;
        snprintf(path, sizeof(path), "%s/table_%s.f32", outdir, tabs[i].name);
        f = fopen(path, "wb");
        if (!f) { perror(path); exit(1); }
        fwrite(tabs[i].buf, sizeof(float), tabs[i].count, f);
        fclose(f);
    }
}
