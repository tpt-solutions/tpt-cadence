//! Decodes a FLAC file to raw interleaved f32 on stdout:
//! `cargo run -p tpt-av-cadence-flac --example flac_decode -- album.flac > out.f32`
...
fn main() {
    let path = std::env::args().nth(1).expect("usage: decode <album.flac>");
    let file = File::open(&path).expect("open input");
    let mut reader = FlacReader::open(Box::new(file)).expect("open flac stream");
    let channels = reader.info().channels as usize;
    eprintln!(
        "flac: {} Hz, {} ch, {}-bit, total_frames {:?}",
